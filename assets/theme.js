  ...
  theme.VideoManager = new function () {
    ...
    _._loadMp4Videos = function (container) {
      $('.video-container[data-video-type="mp4"]:not(.video--init)', container).addClass('video--init').each(function () {
        ...
        $video.on('click mouseenter', () => $video.attr());
        if (autoplay) {
    ...
    $(document).on('load-product-form', '.product-form', function (e) {
      ...
      $(this).on('variantChanged', function (evt, variant, product) {
        ...
        if (variant.title == "MADE TO MEASURE (+£45)"){
          ...
          if (!mtmLineItemProperties) {
          
          // Function to create the line item properties HTML
            function createLineItemProperties() {
              // Create input fields for all 8 line item properties
              var propertiesHTML = `
                <div class="mtm-line-item-properties">
                  <div class="form-field">
                    <label for="chestmtm">Chest:</label>
                    <input type="number" min="0" value="" id="chestmtm" name="properties[Chest]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="heightmtm">Height:</label>
                    <input type="number" min="0" value="" id="heightmtm" name="properties[Height]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="naturalwaistmtm">Natural Waist:</label>
                    <input type="number" min="0" value="" id="naturalwaistmtm" name="properties[Natural Waist]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="weightmtm">Weight:</label>
                    <input type="number" min="0" value="" id="weightmtm" name="properties[Weight]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="lowerwaistmtm">Lower Waist:</label>
                    <input type="number" min="0" value="" id="lowerwaistmtm" name="properties[Lower Waist]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="shouldermtm">Shoulder:</label>
                    <input type="number" min="0" value="" id="shouldermtm" name="properties[Shoulder]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="hipsmtm">Hips:</label>
                    <input type="number" min="0" value="" id="hipsmtm" name="properties[Hips]" pattern="\\d*"/>
                  </div>
            
                  <div class="form-field">
                    <label for="sleevesmtm">Sleeves:</label>
                    <input type="number" min="0" value="" id="sleevesmtm" name="properties[Sleeves]" pattern="\\d*"/>
                  </div>
                  <p>* If not sure about measurements, you can provide measurements later on email </p>
                </div>
              `;
          
              // Get the .product-purchase-form element
              var productPurchaseForm = document.querySelector('.product-purchase-form');
          
              // Prepend the properties HTML to the .product-purchase-form
              productPurchaseForm.insertAdjacentHTML('afterbegin', propertiesHTML);
            }
            }
          
            // Call the function to create and insert the line item properties
            createLineItemProperties();
          }
        else{
          // Find the element with class .mtm-line-item-properties
          var mtmLineItemProperties = document.querySelector('.mtm-line-item-properties');
          
          // Check if the element exists before attempting to remove it
          if (mtmLineItemProperties) {
            // Remove the element from the DOM
            mtmLineItemProperties.parentNode.removeChild(mtmLineItemProperties);
          } 
        }
